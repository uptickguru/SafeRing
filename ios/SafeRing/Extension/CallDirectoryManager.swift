...
final class CallDirectoryManager {
    ...
    /// Opens the Settings app directly to Phone > Call Blocking & Identification.
    /// Users must manually toggle SafeRing ON from there.
    /// Falls back to main Settings if the deep link isn't available.
    @MainActor
    func openSettings() {
        // Try Phone settings directly via deep link
